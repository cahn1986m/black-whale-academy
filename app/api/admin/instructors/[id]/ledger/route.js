...
// add_instructor_ledger_entry() already supports free-reason manual
// entries (entry_type 'payment', any amount/note) — same shape as the
// freelancer ledger below, it was just never exposed via an API route
// until now. A negative 'payment' amount is how a free-reason deduction
// is recorded (mirrors the freelancer ledger's "outgoing/correction"
// direction, not a new entry_type).
export async function GET(request, { params }) {
  const instructorId = Number(params.id);

  const entries = await sql`
    SELECT id, entry_type, amount, balance_after, note, related_attendance_id, reversed_entry_id, created_at
  ...
  const [balanceRow] = await sql`
    SELECT current_balance FROM instructor_balances WHERE instructor_id = ${instructorId}
  `;
...
export async function POST(request, { params }) {
  ...
  if (entryType === 'reversal') {
    const [originalEntry] = await sql`
      SELECT amount FROM instructor_ledger_entries
      WHERE id = ${reversedEntryId} AND instructor_id = ${instructorId}
    `;
  ...
  try {
    await sql`SELECT add_instructor_ledger_entry(${instructorId}, ${entryType}, ${amount}, ${note}, ${reversedEntryId})`;
