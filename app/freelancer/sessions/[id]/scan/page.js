...
export default function FreelancerSessionScanPage({ params }) {
  ...
  useEffect(() => {
    fetch(`/api/freelancer/sessions/${sessionId}`, { cache: 'no-store' })
      .then(async (res) => {
  ...
  const onDecoded = async (text) => {
    ...
    try {
      const res = await fetch(`/api/freelancer/sessions/${sessionId}/scan`, {
        method: 'POST',
  ...
  return (
    <div className="page">
      <Link href={`/freelancer/sessions/${sessionId}`} className="back-link">← تفاصيل الجلسة</Link>
      ...
      {!loading && session && !canScan && (
        <>
          ...
          <Link href={`/freelancer/sessions/${sessionId}`} className="btn secondary" style={{ display: 'flex' }}>
            رجوع لتفاصيل الجلسة
          </Link>
        </>
      )}
