...
export default function RegisterForm() {
  ...
  // Root cause of the earlier bug: this page had no metadata of its own,
  // so Next re-applied the ROOT layout's generic title on every
  // navigation (including client-side <Link> clicks) as part of its own
  // metadata sync — a client-only useEffect here only won that race
  // ~1s later, which is why a quick glance right after navigating still
  // showed the stale title. Fixed by giving page.js its own
  // generateMetadata() (server-rendered, no race). This effect now only
  // has to handle the one thing generateMetadata can't: the visitor
  // toggling locale via the in-page switcher while already on this page.
  useEffect(() => {
