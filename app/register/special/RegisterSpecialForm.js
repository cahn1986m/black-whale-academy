...
// Reuses the exact same 4-step form as /register; the only differences are
// has_special_needs is fixed to true (not asked as a yes/no question) and
// its details field is always visible and required, instead of the
// admin-only conditional field used elsewhere in the app. See page.js for
// why this is split into a server component + this client component.
export default function RegisterSpecialForm() {
  ...
  // Step 3 — medical + special needs (special needs details always shown/required here)
  ...
  // The initial tab title comes from page.js's generateMetadata() (server-
  // rendered). This effect only handles a locale toggle while already on
  // this page — a pure client-side action with no new navigation.
  useEffect(() => {
  ...
  const validateStep3 = () => {
    ...
    if (!specialNeedsDetails.trim()) return t('errSpecialNeedsDetailsRequired');
    return '';
  };
    ...
    try {
      const res = await fetch('/api/register', {
        ...
        body: JSON.stringify({
          ...
          specialNeedsDetails: specialNeedsDetails.trim(),
  ...
  if (result) {
    return (
      <div className="page">
        ...
        <a href="/register/special" className="btn secondary">{t('registerAnother')}</a>
      </div>
    );
  }
        ...
        {step === 3 && (
          <div className="card">
            <div className="field">
              ...
              <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 6 }}>{t('specialNeedsDetailsNote')}</div>
              <input
                type="text"
