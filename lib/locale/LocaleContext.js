...
export function LocaleProvider({ initialLocale, children }) {
  ...
  const setLocale = useCallback((next) => {
    ...
    // The <title> tag itself only updates via generateMetadata() on a real
    // server navigation — toggling locale is a client-side action that
    // never triggers one, so the browser tab title would otherwise stay
    // stale in the old language for the rest of the current page's life.
    document.title = common[value]?.siteTitle || document.title;
  }, []);
