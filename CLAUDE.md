...
Next.js 14 App Router app (JS, no TypeScript), Arabic RTL UI (`app/layout.js` sets `lang="ar" dir="rtl"`), deployed on Vercel with Neon serverless Postgres. Sports-academy attendance + subscription tracker — **no authentication anywhere**; every route (including `/admin` and the destructive reset endpoint) is public to anyone with the URL.
...
app/
  ...
  admin/page.js                    activities CRUD (+ packages), child roster with expandable enrollment
                                    management (renew/unenroll/add), reset-all button
  admin/child/[id]/page.js         printable QR badge for one child (unaffected by activities/enrollments)
